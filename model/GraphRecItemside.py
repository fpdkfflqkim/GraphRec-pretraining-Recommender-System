...
class Model(nn.Module):
    def __init__(self, num_users, num_items, num_ratings, embedding_dict, history_u, history_i, history_ur, history_ue,\
        ...
        self.enc_u = UI_Aggregator_rating(i2e, r2e, u2e, embed_dim, history_u, history_ur, cuda, user=True)
        self.enc_i = UI_Aggregator_review(i2e, emb_dict, u2e, embed_dim, history_i, history_ie, cuda, user=False)
...
class UI_Aggregator_rating(nn.Module):
    """
...
class UI_Aggregator_review(nn.Module):
    """
    item and user aggregator: for aggregating embeddings of neighbors (item/user aggreagator).
    """

...
from model import GraphRecUserside
...
class GraphRec:
    ...
    def _load_data(self):
        ...
        for i in range(len(origin_df)):
            ...
            embedding_mapper[emb_idx] = (
                origin_df[origin_df["embedding_idx"] == emb_idx]["embedding"]
                .item()
                .to(self.args.device)
            )
    ...
    def train_and_infer_by_hit(self):
        ...
        val_loader = torch.utils.data.DataLoader(
            validset, batch_size=self.args.batch_size, shuffle=True
        )
        test_loader = torch.utils.data.DataLoader(
            testset, batch_size=self.args.batch_size, shuffle=True
        )
        ...
        model = GraphRecUserside.Model(
            ...
            self.args.embed_dim,
            social_neighbor,
            cuda=self.args.device,
        ).to(self.args.device)
        optimizer = torch.optim.RMSprop(model.parameters(), lr=self.args.lr, alpha=0.9)
        scheduler = StepLR(
            optimizer, step_size=self.args.lr_dc_step, gamma=self.args.lr_dc
        )
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            for i, data in enumerate(train_loader):
                ...
                optimizer.zero_grad()
                loss = model.loss(
                    batch_nodes_u.to(self.args.device),
                    batch_nodes_i.to(self.args.device),
                    batch_ratings.to(self.args.device),
                )
                loss.backward()
                optimizer.step()
                ...
                if i % 10 == 0:
                    logger.info(
                        f"[Epoch: {epoch}, Iter: {i}] Train Loss: {running_loss / 10:.3f}"
                    )
                    running_loss = 0.0
            ...
            if best_hits < val_hits:
                ...
                endure_count = 0
                torch.save(
                    model, os.path.join(self.args.save_path, "best_checkpoint.pt")
                )
            else:
                endure_count += 1
            ...
            logger.info(
                f"best test HITS@10:{best_test_hits:.4f}, best test MAP@10:{best_test_mapk:.4f}"
            )
    ...
    def rank_test(self, model, test_data):
        ...
        for u in test_data:
            ...
            with torch.no_grad():
                ...
                rank = np.argsort(
                    -np.hstack((scores_pos.cpu().numpy(), scores_neg.cpu().numpy()))
                )
                rank = rank[:10]
    ...
    def test(self, model, test_loader):
        ...
        with torch.no_grad():
            for test_u, test_i, test_ratings in test_loader:
                test_u, test_i, test_ratings = (
                    test_u.to(self.args.device),
                    test_i.to(self.args.device),
                    test_ratings.to(self.args.device),
                )
